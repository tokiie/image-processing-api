...
describe('File Validation Tests', () => {
  ...
  describe('Invalid File Formats', () => {
    ...
    it('should reject requests with image file too small', async () => {
      const response = await request(app)
        .post('/api/image-processing/jobs')
        .attach('image', path.join(testImagesDir, 'test_image_100x100.png'))
        .field('userId', validUserId);

      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty('error');
      expect(response.body.error).toBe('Invalid image');
    });
  });
